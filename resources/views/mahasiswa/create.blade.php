                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5">
                    <div>
                        ...
                        <select name="asal_kampus" class="searchable-select w-full bg-white border border-slate-200 rounded-lg px-4 py-2.5 text-sm text-slate-800 focus:border-primary focus:ring-2 focus:ring-primary/40 outline-none transition-all cursor-pointer" required>
                            <option value="" disabled selected>Pilih Asal Kampus</option>
                            @foreach($instansis as $instansi)
                                <option value="{{ $instansi->nama_instansi }}" {{ old('asal_kampus') == $instansi->nama_instansi ? 'selected' : '' }}>{{ $instansi->nama_instansi }}</option>
                            @endforeach
                        </select>
                    </div>
