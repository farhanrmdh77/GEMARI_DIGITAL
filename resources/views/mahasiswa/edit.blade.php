            ...
            <section>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5">
                    <div>
                        ...
                        <select name="asal_kampus" class="searchable-select w-full bg-white border border-slate-200 rounded-lg px-4 py-2.5 text-sm text-slate-800 focus:border-primary focus:ring-2 focus:ring-primary/40 outline-none transition-all cursor-pointer" required>
                            <option value="" disabled>Pilih Asal Kampus</option>
                            @foreach($instansis as $instansi)
                                <option value="{{ $instansi->nama_instansi }}" {{ old('asal_kampus', $mahasiswa->asal_kampus) == $instansi->nama_instansi ? 'selected' : '' }}>{{ $instansi->nama_instansi }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="sm:col-span-2 mt-2">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <label class="relative cursor-pointer block">
                                <input type="radio" name="jenis_magang" value="Reguler" class="peer sr-only" required {{ old('jenis_magang', $mahasiswa->jenis_magang) == 'Reguler' ? 'checked' : '' }}>
                            ...
                            <label class="relative cursor-pointer block">
                                <input type="radio" name="jenis_magang" value="Berbayar" class="peer sr-only" required {{ old('jenis_magang', $mahasiswa->jenis_magang) == 'Berbayar' ? 'checked' : '' }}>
                                <div class="absolute inset-0 border-2 border-slate-200 rounded-xl transition-all peer-checked:border-primary peer-checked:bg-primary/5 peer-hover:border-slate-300 pointer-events-none"></div>
                                <div class="relative p-4 flex items-center pointer-events-none z-10">
                                    <div class="flex-1 pr-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 border-t border-slate-100 pt-8 mt-4">
                <div>
                    ...
                    @if($mahasiswa->surat_permohonan)
                        <div class="mb-3 flex items-center justify-between p-3 bg-primary/5 border border-primary/20 rounded-lg">
                            <a href="{{ route('dokumen.view', ['path' => $mahasiswa->surat_permohonan]) }}" target="_blank" class="text-xs font-medium text-primary hover:underline flex items-center gap-2">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path></svg>
                                Lihat File Tersimpan
                            </a>
                            <div class="flex items-center gap-2">
                                <input type="checkbox" name="hapus_surat" value="1" id="hapus_surat" class="w-4 h-4 text-red-500 rounded border-slate-300 focus:ring-red-500 cursor-pointer">
                                <label for="hapus_surat" class="text-xs text-red-500 font-medium cursor-pointer">Hapus</label>
                            </div>
                        </div>
                    @endif
                    ...
                    <p class="text-xs text-slate-500 mt-2">Ganti file (Opsional). Format: PDF (Max: 5MB)</p>
                </div>
                <div>
                    <h3 class="text-[15px] font-bold font-geist text-primary mb-4 flex items-center gap-2">
                    ...
                    @if($mahasiswa->laporan_magang)
                        <div class="mb-3 flex items-center justify-between p-3 bg-primary/5 border border-primary/20 rounded-lg">
                            <a href="{{ route('dokumen.view', ['path' => $mahasiswa->laporan_magang]) }}" target="_blank" class="text-xs font-medium text-primary hover:underline flex items-center gap-2">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path></svg>
                                Lihat File Tersimpan
                            </a>
                            <div class="flex items-center gap-2">
                                <input type="checkbox" name="hapus_laporan" value="1" id="hapus_laporan" class="w-4 h-4 text-red-500 rounded border-slate-300 focus:ring-red-500 cursor-pointer">
                                <label for="hapus_laporan" class="text-xs text-red-500 font-medium cursor-pointer">Hapus</label>
                            </div>
                        </div>
                    @endif
                    <input type="file" name="laporan_magang" class="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-800 focus:border-primary outline-none transition-all cursor-pointer file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-primary/10 file:text-primary hover:file:bg-primary/20" accept=".pdf">
                    <p class="text-xs text-slate-500 mt-2">Ganti file (Opsional). Format: PDF (Max: 10MB)</p>
                </div>
            </div>
